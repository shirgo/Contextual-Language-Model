...
if train_flag==0:
    ...
    in_text = 'i'  # input text
    model_file = r"model.hdf5"
...
def build_model(input_len ,total_words, embedding_dims, batch_size):
    input_seq = Input(shape=[input_len])
    Emb_x = Embedding(total_words, embedding_dims, input_length=input_len)(input_seq)
    W_Emb_x = Dense(embedding_dims, activation='softmax')(Emb_x)

    input_label = Input(shape=[input_len])
    Emb_label = Embedding(total_labels, embedding_dims, input_length=input_len)(input_label)
    W_Emb_label = Dense(embedding_dims, activation='softmax')(Emb_label)

    lstm_in = Add()([W_Emb_x, W_Emb_label])

    # different options:
    # lstm_out = CuDNNLSTM(rnn_cell_size)(lstm_in)
    # lstm_out = LSTM(rnn_cell_size)(lstm_in)
    # lstm_out = LSTM(rnn_cell_size, dropout=dropout)(lstm_in)
    # lstm_out = GRU(rnn_cell_size)(lstm_in)
    lstm_out = LSTM(rnn_cell_size)(lstm_in)
...
if train_flag:
    ...
    plotConvergence(history)
        ...
        print("Created model and loaded weights from file: " + model_file)
